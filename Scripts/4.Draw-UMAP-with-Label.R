...
labelNew <- mapLabels(proj$Clusters_Tile, oldLabels=names(remapClust),newLabels=remapClust)
proj$Labels <- labelNew
getGroupBW(ArchRProj=proj,groupBy="Labels")

# Make Umap with labels
png(paste0("Labeled-UMAP.",Accession,".png"),width=10,height=8,units="in",res=300)
data <- data.frame(table(proj$Labels))
...
barplot <- ggplot(data=data,aes(x=cellType,y=count,fill=cellType))+
           ...
           ggtitle(args[2])+
           coord_flip()
umap <- ggplot(p,aes(x=p[,1], y=p[,2], color=proj$Labels))+
        geom_point()+
